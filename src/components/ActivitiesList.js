    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {displayedActivities.map((activity) => (
          <div key={activity.CODACTIVIDAD} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
              ...
              />
            <div className="p-6">
              <h3 className="text-xl font-bold text-blue-700 mb-2">{activity.NOMACTIVIDAD}</h3>
              <p className="text-blue-500 text-mb font-semibold mb-3">
                {new Date(activity.FECACTIVIDAD).toLocaleDateString()}

              </p>
              <p className="text-blue-600 font-semibold mb-4">{activity.SHORTDESCRIPTION}</p>
              <Link
      ...
      {!showAll && activities.length > 3 && (
        <div className="mt-8 text-center">
          <Link
            to="/actividades"
            className="inline-block px-6 py-2 border border-violet-600 text-blue-600 rounded-lg hover:bg-blue-50 transition-colors"
          >
