  ...
  return (
    <>
      ...
      <nav className="bg-blue-600 text-white shadow-lg">
          ...
          <div className="flex justify-between items-center">
            ...
            >
              <img
                src="/logowhite6.png"
                alt="caminantes"
                className="h-full w-32 md:w-40 object-contain"
              />
            </Link>
            {/* Hamburger Menu Button (visible on small screens) */}
            <div className="md:hidden flex items-center">
              {user && ( // Show connected user info on mobile if logged in
                <div className="flex flex-col text-right text-md leading-tight mr-4">
                  <span className="font-semibold">
                    {getInitials(user.name)}
                  </span>
                  <span className="text-blue-200 capitalize text-xs">
                    ({user.role})
                  </span>
                </div>
              )}
            ...
            <div className="hidden md:flex items-center space-x-6">
              <Link
                to="/"
                className="hover:text-blue-200 transition-colors text-lg"
              >
                Actividades
              </Link>
              <Link
              ...
              {user?.role === "admin" && (
                <div className="relative" ref={adminMenuRef}>
                  <button
                    ...
                    className="hover:text-blue-200 transition-colors flex items-center focus:outline-none text-lg"
                  ...
                  {adminMenuOpen && (
                    ...
                    >
                      <Link
                        to="/admin"
                        className="block px-4 py-2 text-md text-blue-800 hover:bg-blue-100 transition-colors"
                        role="menuitem"
                        onClick={() => {
                          setAdminMenuOpen(false);
                          closeMobileMenu();
            ...
            <div className="hidden md:flex items-center space-x-4">
              {user ? (
                <>
                  <div className="flex flex-col text-right text-md leading-tight">
                  ...
                  <button
                    ...
                    className="bg-blue-700 hover:bg-blue-800 px-4 py-2 rounded-lg transition-colors text-lg font-medium"
                  >
                    Cerrar Sesión ➡️
                  </button>
                </>
        ...
        {mobileMenuOpen && (
          <div
            className="md:hidden bg-blue-700 pb-4 shadow-inner"
            ref={mobileNavRef}
          >
            <div className="flex flex-col items-center space-y-4 pt-2">
              <Link
                to="/"
                className="block text-white hover:text-blue-200 transition-colors text-lg"
              ...
              {user?.role === "admin" && (
                <>
                  ...
                  <Link
                    to="/admin"
                    className="block px-4 py-2 text-base text-blue-100 hover:bg-blue-800 rounded-md transition-colors w-full text-center"
                    onClick={closeMobileMenu}
                  >
                    Dashboard
                  </Link>
              ...
              {user ? (
                <>
                  ...
                  <button
                    onClick={handleLogout}
                    className="bg-blue-800 hover:bg-blue-900 px-4 py-2 rounded-lg transition-colors text-lg font-medium w-3/4"
                  >
                    Cerrar Sesión ➡️
                  </button>
                </>
